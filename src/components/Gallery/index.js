...
class Gallery extends Component {
  ...
  render() {
    ...
    const checkList = imagesList.filter(eachObj => eachObj.imageUrl === imgUrl)

    const {id} = checkList[0]

    return (
      <div className="gallery-container">
        <img src={imgUrl} alt={imgAltText} className="gallery-image" />
        ...
        <ul className="thumbnail-container">
          {imagesList.map(eachObj => (
            <ThumbnailItem
              key={eachObj.id}
              imageObject={eachObj}
              onClickButton={this.onClickThumbnail}
              id={id}
            />
          ))}
        </ul>
